...
function handle_booking_ams(WP_REST_Request $request)
{
    ...
    for ($i = 0; $i < $totalTickets; $i++) {
        ...
        if (!empty($departure_dates_req[$i])) {
            update_post_meta($post_id, 'departure_date', sanitize_text_field($departure_dates_req[$i]) ?: '');
        }
...
function handle_get_review_ajax_company()
{
    ...
    $url = endPoint . "/api/bus-operation/$partnerName/$companyId/reviews?page=$page&limit=10";
    ...
    if (is_wp_error($response)) {
        ...
        if (!empty($data) && is_array($data)) {
            foreach ($data['data']['reviews'] as $item) {
                $dateString = isset($item['tripDate']) ? date('d-m-Y', strtotime($item['tripDate'])) : "";
                ...
                if (!empty($item['social_avatar'])) {
                    $output .= '<div class="rating-tab__comments-list__item-personal_social-avatar"><img src="' . $item['social_avatar'] . '" alt="' . $item['reviewer_name'] . '"></div>';
                } else {
                    $output .= '<div class="rating-tab__comments-list__item-personal__info-avatar">' . getInitialsNameToAvatar($item['reviewer_name']) . '</div>';
                }
...
function handle_get_info_ajax_company()
{
    ...
    if ($partnerId === 'vexere') {
        ...
        $params = array(
            "routeId"      => $tripCode,
            "departureTime"   => $pickupDate ? str_replace(':', 'h', $pickupDate) : '',
            "wayId"       => $wayId,
            "bookingId"  => $bookingId,
        );
        ...
        $responseSeatInfo  = call_api_v2("trip/get-trip-detail", "POST", $body);
    }
...
function handle_get_bus_amenities()
{
    ...
    if (empty($trip_code) || empty($partner_id)) {
        wp_send_json_error('Thiếu tham số trip_code / partnerId');
    }

    $url  = endPoint . "/api/bus-amenity";
    $body = [
        "trip_code" => $trip_code,
        "partnerId" => $partner_id,
    ];

    $response = call_api_with_token_agent($url, 'POST', $body);
    ...
    if (
        empty($data) ||
        !isset($data['data']['busAmenities']) ||
        !is_array($data['data']['busAmenities'])
    ) {
        wp_send_json_error('Không có dữ liệu tiện ích từ API. Raw: ' . json_encode($data));
    }

    $amenities = $data['data']['busAmenities'];
...
function handle_get_policy_mapping()
{
    ...
    $url = endPoint . "/api/company/get-policy";
    $body = [
        "trip_code"        => $tripCode,
...
function handle_get_cancellation_policy()
{
    ...
    $url  = endPoint . "/api/cancellation-policy/";
    $body = [
        "partnerId"     => $partnerId,
        "trip_code"     => $trip_code,
    ...
    if (
        empty($raw['data'])
        || empty($raw['data']['data'])
        || !is_array($raw['data']['data'])
    ) {
        echo '';
        wp_die();
    }

    $policy  = $raw['data']['data'][0];
    $details = !empty($policy['details']) && is_array($policy['details']) ? $policy['details'] : [];
